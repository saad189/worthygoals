...
export class MentorImprovement1767604205672 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        if (!hasSortOrder) {
            await queryRunner.query(`ALTER TABLE \`mentors\` ADD \`sortOrder\` int NOT NULL DEFAULT '0'`);
        }

        const hasModelConfig = await queryRunner.hasColumn('mentors', 'modelConfig');
        if (!hasModelConfig) {
            await queryRunner.query(`ALTER TABLE \`mentors\` ADD \`modelConfig\` json NULL`);
        }

        // Idempotent: re-applying the same enum definition is safe.
        await queryRunner.query(`ALTER TABLE \`mentors\` CHANGE \`communicationStyle\` \`communicationStyle\` enum ('gentle', 'analytical', 'motivational', 'direct', 'straightforward', 'empathetic', 'calm', 'energetic', 'clear') NOT NULL DEFAULT 'gentle'`);
        ...
        if (!existingIndex?.length) {
            await queryRunner.query(`CREATE INDEX \`IDX_1d74e5008c837e06cd4da88d28\` ON \`mentors\` (\`sortOrder\`)`);
        }
    }
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        if (idx?.length) {
            await queryRunner.query(`DROP INDEX \`IDX_1d74e5008c837e06cd4da88d28\` ON \`mentors\``);
        }

        await queryRunner.query(`ALTER TABLE \`mentors\` CHANGE \`communicationStyle\` \`communicationStyle\` enum ('gentle', 'analytical', 'motivational', 'direct') NOT NULL DEFAULT 'gentle'`);
