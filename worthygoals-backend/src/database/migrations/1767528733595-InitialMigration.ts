...
export class InitialMigration1767528733595 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE \`message_feedback\` (\`id\` varchar(36) NOT NULL, \`messageId\` varchar(255) NOT NULL, \`userId\` int NOT NULL, \`rating\` smallint NOT NULL, \`tags\` json NULL, \`comment\` text NULL, \`createdAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), INDEX \`idx_message_feedback_user_id\` (\`userId\`), INDEX \`idx_message_feedback_message_id\` (\`messageId\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`CREATE TABLE \`conversations\` (\`id\` varchar(36) NOT NULL, \`userId\` int NOT NULL, \`mentorId\` int NOT NULL, \`status\` varchar(16) NOT NULL DEFAULT 'active', \`title\` varchar(180) NULL, \`lastMessageAt\` datetime NULL, \`lastMessageId\` varchar(255) NULL, \`metadata\` json NULL, \`createdAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updatedAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), INDEX \`idx_conversations_user_id\` (\`userId\`), INDEX \`idx_conversations_mentor_id\` (\`mentorId\`), INDEX \`idx_conversations_mentor_last_message_at\` (\`mentorId\`, \`lastMessageAt\`), INDEX \`idx_conversations_user_last_message_at\` (\`userId\`, \`lastMessageAt\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`accounts\` (\`id\` int NOT NULL AUTO_INCREMENT, \`sub\` varchar(255) NOT NULL, \`email\` varchar(255) NOT NULL, \`isSignUp\` tinyint NULL DEFAULT 0, \`dateAdded\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`dateUpdated\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), UNIQUE INDEX \`IDX_59d5ec9ccb3f6d03a52605f892\` (\`sub\`), UNIQUE INDEX \`IDX_ee66de6cdc53993296d1ceb8aa\` (\`email\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`ALTER TABLE \`message_feedback\` ADD CONSTRAINT \`FK_935ceb641a40fe337716a57a612\` FOREIGN KEY (\`userId\`) REFERENCES \`users\`(\`id\`) ON DELETE CASCADE ON UPDATE NO ACTION`);
        ...
        await queryRunner.query(`ALTER TABLE \`messages\` ADD CONSTRAINT \`FK_4838cd4fc48a6ff2d4aa01aa646\` FOREIGN KEY (\`userId\`) REFERENCES \`users\`(\`id\`) ON DELETE SET NULL ON UPDATE NO ACTION`);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`DROP INDEX \`IDX_59d5ec9ccb3f6d03a52605f892\` ON \`accounts\``);
