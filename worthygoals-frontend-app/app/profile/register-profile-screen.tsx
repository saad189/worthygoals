...
export default function RegisterProfileScreen() {
    ...
    const [items, setItems] = useState([{ label: 'Male', value: 'm' }, { label: 'Female', value: 'f' }]);
    ...
    const onChangeDate = (_event: any, selectedDate?: Date) => {
        setShowDatePicker(Platform.OS === 'ios'); // Keep the picker open on iOS or close on Android
        if (selectedDate) {
    ...
    const validateForm = () => {
        ...
        const dateOfBirthError = !formState.dateOfBirth.value ? 'Select a date of birth' : '';
    ...
    return (
        <Background>
            ...
            >
                <TextInput
                    ...
                    onChangeText={(text: string) => {
                        ...
                        dispatch({ type: 'UPDATE_DATE_OF_BIRTH', payload: date });
                    }}
                    error={!!formState.dateOfBirth.error}
                    errorText={formState.dateOfBirth.error}
                    description={formState.dateOfBirth.value ? 'You are ' + calculateAge(formState.dateOfBirth.value.toDateString()) + ' years old' : ''}
                />
            </TouchableOpacity>
            ...
            <Button mode="contained" onPress={onSubmit} style={styles.button} loading={isLoading}>
                Submit
            </Button>
